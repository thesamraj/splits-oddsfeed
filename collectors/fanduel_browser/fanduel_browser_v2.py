#!/usr/bin/env python3
"""
FanDuel Browser Automation Collector V2
Simplified version focusing on main page scraping
"""
...
class FanDuelBrowserCollector:
    ...
    async def extract_live_odds(self):
        """Extract odds from FanDuel main page"""
        odds_data = []

        try:
            logger.info("Navigating to FanDuel...")
            # Go to main sportsbook page
            ...
            # Try to find and click live section
            try:
                live_selectors = [
                    'text="Live Betting"',
                    'text="LIVE"',
            ...
            # Extract events with multiple selector strategies
            event_selectors = [
                '[data-test-id*="event"]',
                'div[class*="EventCard"]',
            ...
            for selector in event_selectors:
                try:
                    ...
                    if events:
                        ...
                        for event in events[:10]:  # Process max 10 events
                            try:
                                event_data = await self.page.evaluate(
                                    ...
                                    // Find team names with multiple strategies
                                    let teams = [];

                                    // Try various team selectors
                                    const teamSelectors = [
                                        'span[role="text"]',
                                        '[class*="team"]',
                                        '[class*="competitor"]',
                                    ...
                                    for (const selector of teamSelectors) {
                                        ...
                                        if (found.length >= 2) {
                                            ...
                                            const potential = Array.from(found)
                                    ...
                                    const oddsElements = element.querySelectorAll(
                                        'button[aria-label*="odds"], [class*="odds"], [class*="price"]'
                                    );
                                    const odds = [];
                                ...
                                if (
                                    len(event_data["teams"]) >= 2
                                    and len(event_data["odds"]) > 0
                                ):
                                    event_id = f"fd_{event_data['teams'][0]}_{event_data['teams'][1]}".replace(
                                        " ", "_"
                                    )
    ...
    async def run(self):
        ...
        logger.info("Starting FanDuel browser collector V2")
