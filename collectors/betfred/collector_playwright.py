#!/usr/bin/env python3
"""BetFred Playwright collector with XHR intercept"""
...
async def intercept_xhr():
    ...
    async with async_playwright() as p:
        ...
        async def handle_request(route, request):
            url = request.url
            if 'api' in url and any(x in url for x in ['event', 'odds', 'sport', 'market']):
        ...
        try:
            # Navigate to BetFred US
            for url in ['https://pa.betfred.com', 'https://sports.betfred.com']:
                try:
            ...
            # Click into sports sections
            for selector in ['a:has-text("NFL")', 'a:has-text("Football")', '[href*="american-football"]']:
                try:
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if isinstance(data, dict):
                        raw_events = data.get('events', data.get('fixtures', []))
                    ...
                    for evt in raw_events[:10]:
                        try:
                            event = {
                                'event_id': f"betfred_{evt.get('id', int(time.time()*1000))}",
                                'home': evt.get('homeTeam', {}).get('name', 'Home'),
                                'away': evt.get('awayTeam', {}).get('name', 'Away'),
