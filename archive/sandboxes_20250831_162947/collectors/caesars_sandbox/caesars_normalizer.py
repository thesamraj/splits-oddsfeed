...
def ensure_event(cursor, event_id, home, away):
    ...
    try:
        cursor.execute(
            ...
            VALUES (%s, 'TBD', NOW(), %s, %s, 'TBD', 'caesars', NOW())
...
def process_message(channel, data):
    """Process incoming Caesars odds data"""
    try:
                ...
                if market == "h2h":
                    ...
                    if home_price and away_price and line is not None:
                        cursor.execute(
                            ...
                            VALUES (%s, 'caesars', 'spreads', %s, %s, %s, NOW())
                        """,
                    ...
                    if over_price and under_price and total is not None:
                        cursor.execute(
                            ...
                            VALUES (%s, 'caesars', 'totals', %s, %s, %s, NOW())
                        """,
                            (event_id, over_price, under_price, total),
                        )

        cursor.close()
        print(f"Processed {len(events)} Caesars events", flush=True)
...
def main():
    """Main loop to listen for Caesars odds data"""
    print(f"Caesars normalizer started at {datetime.now()}", flush=True)

    # Subscribe to Caesars raw odds channel
    pubsub = r.pubsub()
    pubsub.subscribe("odds.raw.caesars")
