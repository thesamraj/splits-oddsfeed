...
def kambi_fallback_extract_betrivers(payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for event_node in live_events:
            ...
            # Extract event_id
            event_id = None
            for id_path in [["event", "id"], ["id"], ["eventId"], ["event_id"]]:
                try:
                    val = event_node
                    for key in id_path:
                        val = val[key]
            ...
            for market in market_containers:
                ...
                market_name = (
                    market.get("name", "") or market.get("criterionLabel", "")
                ).lower()
                if (
                    "moneyline" in market_name
                    or "winner" in market_name
                    or "match" in market_name
                ):
                    market_type = "h2h"
                ...
                for i, outcome in enumerate(outcomes):
                    ...
                    logger.info(
                        "FALLBACK_EMIT event_id=%s market=%s side=%s price=%s",
                        event_id,
                        market_type,
                        side,
                        price_american,
        ...
        logger.info(
            "FALLBACK_SCAN liveEvents=%d candidates=%d outcomes=%d",
            len(live_events),
            markets_found,
            outcomes_found,
        )
