#!/usr/bin/env python3
"""Adapter for Circa sportsbook messages"""
...
class CircaAdapter(BaseAdapter):
    """Convert Circa messages to canonical format"""
    
    def __init__(self):
        super().__init__("circa")
    
    def to_canonical(self, raw_message: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Convert Circa raw message to canonical format"""
        try:
            ...
            for event in events:
                canonical = {
                    ...
                    "event_id": event.get("event_id", f"circa_{int(time.time()*1000)}"),
                ...
                if self.validate_canonical(canonical):
                    ...
                    logger.warning(f"Circa event failed validation: {event.get('event_id', 'unknown')}")
            ...
            logger.error(f"Circa adapter error: {e}")
            return None