#!/usr/bin/env python3
"""Adapter for ubetway sportsbook messages"""
...
class ubetwayAdapter(BaseAdapter):
    """Convert ubetway messages to canonical format"""
    
    def __init__(self):
        super().__init__("betway")
    
    def to_canonical(self, raw_message: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Convert ubetway raw message to canonical format"""
        try:
            ...
            for event in events:
                canonical = {
                    ...
                    "event_id": event.get("event_id", f"betway_{int(time.time()*1000)}"),
                ...
                if self.validate_canonical(canonical):
                    ...
                    logger.warning(f"ubetway event failed validation: {event.get('event_id', 'unknown')}")
            ...
            logger.error(f"ubetway adapter error: {e}")
            return None