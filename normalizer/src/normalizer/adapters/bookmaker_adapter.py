#!/usr/bin/env python3
"""Adapter for ubookmaker sportsbook messages"""
...
class ubookmakerAdapter(BaseAdapter):
    """Convert ubookmaker messages to canonical format"""
    
    def __init__(self):
        super().__init__("bookmaker")
    
    def to_canonical(self, raw_message: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Convert ubookmaker raw message to canonical format"""
        try:
            ...
            for event in events:
                canonical = {
                    ...
                    "event_id": event.get("event_id", f"bookmaker_{int(time.time()*1000)}"),
                ...
                if self.validate_canonical(canonical):
                    ...
                    logger.warning(f"ubookmaker event failed validation: {event.get('event_id', 'unknown')}")
            ...
            logger.error(f"ubookmaker adapter error: {e}")
            return None