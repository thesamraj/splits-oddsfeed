#!/usr/bin/env python3
"""Adapter for ubetonline sportsbook messages"""
...
class ubetonlineAdapter(BaseAdapter):
    """Convert ubetonline messages to canonical format"""
    
    def __init__(self):
        super().__init__("betonline")
    
    def to_canonical(self, raw_message: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Convert ubetonline raw message to canonical format"""
        try:
            ...
            for event in events:
                canonical = {
                    ...
                    "event_id": event.get("event_id", f"betonline_{int(time.time()*1000)}"),
                ...
                if self.validate_canonical(canonical):
                    ...
                    logger.warning(f"ubetonline event failed validation: {event.get('event_id', 'unknown')}")
            ...
            logger.error(f"ubetonline adapter error: {e}")
            return None