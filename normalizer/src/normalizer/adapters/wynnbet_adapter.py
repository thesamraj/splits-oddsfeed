#!/usr/bin/env python3
"""Adapter for uwynnbet sportsbook messages"""
...
class uwynnbetAdapter(BaseAdapter):
    """Convert uwynnbet messages to canonical format"""
    
    def __init__(self):
        super().__init__("wynnbet")
    
    def to_canonical(self, raw_message: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Convert uwynnbet raw message to canonical format"""
        try:
            ...
            for event in events:
                canonical = {
                    ...
                    "event_id": event.get("event_id", f"wynnbet_{int(time.time()*1000)}"),
                ...
                if self.validate_canonical(canonical):
                    ...
                    logger.warning(f"uwynnbet event failed validation: {event.get('event_id', 'unknown')}")
            ...
            logger.error(f"uwynnbet adapter error: {e}")
            return None